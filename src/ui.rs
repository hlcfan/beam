                ...
                panel = panel.child(
                    Button::new(format!("tree-row-{}", row_id))
                        ...
                        .context_menu(move |menu, window, _| {
                            ...
                            match row_kind {
                                TreeNodeKind::Collection => {
                                    menu = menu.item(PopupMenuItem::new("Add Request").on_click(
                                        window.listener_for(&view, move |this, _, window, cx| {
                                            this.add_request_from_tree_node(row_id, window, cx);
                                        }),
                                    ));
                                    menu = menu.item(PopupMenuItem::new("Add Folder").on_click(
                                        window.listener_for(&view, move |this, _, window, cx| {
                                ...
                                TreeNodeKind::Request => {
                                    menu = menu.item(PopupMenuItem::new("Send Request").on_click(
                                        window.listener_for(&view, move |this, _, window, cx| {
                                            this.send_request_from_tree_node(row_id, window, cx);
                                        }),
                                    ));
                                    menu = menu.item(PopupMenuItem::new("Copy as cURL").on_click(
                                        window.listener_for(&view, move |this, _, window, cx| {
                                    ...
                                    menu = menu.item(PopupMenuItem::new("Duplicate").on_click(
                                        window.listener_for(&view, move |this, _, window, cx| {
...
fn execute_http_request(request: RequestAuthoringState) -> HttpResponseView {
    ...
    let mut builder = client
        .request(
            http_method_to_reqwest(request.method),
            request.url.trim(),
        )
        .query(&query_pairs);
